...
from gem.analysis.rdkit_functions import compute_molecular_metrics
import torch
...
from gem.metrics.abstract_metrics import compute_ratios
...
class SamplingMolecularMetrics(nn.Module):
    def __init__(self, dataset_infos, dataset_smiles, cfg, add_virtual_states=False):
        ...
        self._disable_metric_sync(
            self.generated_n_dist,
            self.generated_node_dist,
            self.generated_edge_dist,
            self.generated_valency_dist,
            self.n_dist_mae,
            self.node_dist_mae,
            self.edge_dist_mae,
    ...
    @staticmethod
    def _disable_metric_sync(*metrics: Metric) -> None:
        for metric in metrics:
            ...
            if hasattr(metric, "_to_sync"):
                metric._to_sync = False  # type: ignore[attr-defined]
    ...
    def forward(
        ...
        if test and local_rank == 0:
            ...
            if getattr(getattr(self.cfg, "sample", object()), "log_smiles", False):
...
def _resolve_fcd_device(device: str) -> str:
...
@lru_cache(maxsize=4)
def _load_fcd_model(device: str = "cpu"):
    device = _resolve_fcd_device(device)
    try:
        from fcd_torch import FCD

        return "fcd_torch", FCD(device=device, n_jobs=0)
    except ImportError:
...
def _fcd_activations(smiles: Sequence[str], *, device: str = "cpu"):
    device = _resolve_fcd_device(device)
    backend, model = _load_fcd_model(device)
    if backend == "fcd_torch":
        return model.get_predictions(list(smiles))

    from fcd import get_predictions

    return get_predictions(model, list(smiles), n_jobs=0, device=device)


def compute_fcd_statistics(
    smiles: Sequence[str],
    *,
    device: str = "cpu",
) -> tuple[np.ndarray, np.ndarray]:
    activations = np.asarray(_fcd_activations(smiles, device=device))
    if activations.ndim != 2 or activations.shape[0] == 0:
        raise ValueError(
            f"Cannot compute FCD statistics from activations with shape {activations.shape}."
        )
    return np.mean(activations, axis=0), np.cov(activations, rowvar=False)


def compute_fcd_from_statistics(
    reference_statistics: tuple[np.ndarray, np.ndarray],
    ...
    *,
    ...
    mu_val, sigma_val = reference_statistics
    mu_gen, sigma_gen = compute_fcd_statistics(generated_smiles, device=device)

    return calculate_frechet_distance(mu_gen, sigma_gen, mu_val, sigma_val)


def prepare_fcd_smiles(smiles_seq, *, canonicalize: bool = True):
    cleaned = []
    rejected = 0
    for s in smiles_seq or []:
        if s is None:
            rejected += 1
            continue
        ...
        if mol is None:
            rejected += 1
            continue
        cleaned.append(Chem.MolToSmiles(mol) if canonicalize else s)
    return cleaned, rejected


def _clean_fcd_smiles(smiles_seq):
    return prepare_fcd_smiles(smiles_seq, canonicalize=True)
...
def compute_fcd(
    ...
    generated_smiles,
    *,
    canonicalize_reference: bool = True,
    device: str = "cpu",
):
    """Compute FCD after discarding SMILES that RDKit cannot parse."""

    print("Starting FCD computation")
    start = time.time()

    val_smiles, rejected_val = prepare_fcd_smiles(
        val_smiles,
        canonicalize=canonicalize_reference,
    )
    generated_smiles, rejected_generated = prepare_fcd_smiles(
        generated_smiles,
        canonicalize=True,
    )
    if rejected_val or rejected_generated:
        print(
            "Discarded unparsable SMILES before FCD: "
            f"reference={rejected_val}, generated={rejected_generated}"
        )

    if not val_smiles or not generated_smiles:
        print("No SMILES available for FCD computation after cleaning; returning -1.")
        return -1.0

    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        try:
            reference_statistics = compute_fcd_statistics(val_smiles, device=device)
            fcd_score = compute_fcd_from_statistics(
            ...
            traceback.print_exc()
            print("Error in FCD computation. Setting FCD to -1.")
            fcd_score = -1

...
# Track which transitions have had their limit distributions printed
_PRINTED_LIMIT_FOR = set()
...
class NoiseDistribution:
    ...
    def __init__(self, model_transition, dataset_infos):
        ...
        # Print once per transition type to show relevant distributions
        global _PRINTED_LIMIT_FOR
        if model_transition not in _PRINTED_LIMIT_FOR:
            print(
                f"Limit distribution of the classes ({model_transition}) | Nodes: {x_limit} | Edges: {e_limit}"
            )
            _PRINTED_LIMIT_FOR.add(model_transition)

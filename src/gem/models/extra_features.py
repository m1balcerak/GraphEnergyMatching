...
class NodeCycleFeatures:
    ...
    def __call__(self, noisy_data):
        ...
        kcycles = KNodeCycles()
        x_cycles, y_cycles = kcycles.k_cycles(adj_matrix=adj_matrix)  # (bs, n_cycles)
...
class EigenFeatures:
    """
    Code adapted from an external reference implementation.
    """
    ...
    def __call__(self, noisy_data):
        E_t = noisy_data["E_t"]
        mask_bool = noisy_data["node_mask"].bool()
        mask = mask_bool.to(dtype=E_t.dtype)
...
def get_eigenvalues_features(eigenvalues, k=5):
    ...
    if not (n_connected_components > 0).all():
        raise ValueError(
            f"Invalid Laplacian spectrum: n_connected_components={n_connected_components}."
        )
